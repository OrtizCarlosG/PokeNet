...
import org.pokemonium.server.backend.map.ServerMap.PvPType;
...
public class Player extends Character implements Battleable, Tradeable
{
	...
	/*New Clan Stuff */
	private String userClan;
	private String clan_Tag;
	private int clanuserRank;
	...
	/**
	 * Send current player position to be resynchronized
	 */
	public void sendToBeResynchronized()
	{
	...
	{
		ServerMessage MessageMoveOne = new ServerMessage(ClientPacket.PLAYER_TELEPORTBEHAVIOR);
		MessageMoveOne.addBool(false);
		getSession().Send(MessageMoveOne);
		...
		for(Player p : map.getPlayers().values())
		{
			...
			packet = packet + c.getName() + "," + c.getId() + "," + c.getSprite() + "," + c.getX() + "," + c.getY() + ","
					...
					+ c.getFollowPokemonID() + "," + c.getPokemonLastX() + "," + c.getPokemonLastY() + ",";
		}
		...
		/* Release to move */
		ServerMessage MessageMoveTwo = new ServerMessage(ClientPacket.PLAYER_TELEPORTBEHAVIOR);
		MessageMoveTwo.addBool(true);
		getSession().Send(MessageMoveTwo);
	}
	...
	/*New clan Stuff*/
	public void setUserClan(String clan)
	{
		this.userClan = clan;
	}
	...
	public void setUserClanTag(String tag){
		this.clan_Tag = tag;
	}
	...
	public void setUserRank(int rank){
		this.clanuserRank = rank;
	}
	
	public int getUserRank(){
		return this.clanuserRank;
	}
	/*END*/

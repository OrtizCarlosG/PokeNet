...
import org.pokemonium.server.battle.PokemonEgg;
...
public class BreedingLogic
{
	...
	{
		for(int i : PokemonSpecies.getDefaultData().getPokemonByName(poke1.getName()).getCompatibility())
			for(int x : PokemonSpecies.getDefaultData().getPokemonByName(poke2.getName()).getCompatibility())
				if(i == x)
					if(poke1.getGender() == PokemonSpecies.GENDER_MALE && poke2.getGender() == PokemonSpecies.GENDER_FEMALE || poke2.getSpeciesName() == "Ditto")
					{
						malePoke = poke1;
						femalePoke = poke2;
						return true;
					}
					else if(poke2.getGender() == PokemonSpecies.GENDER_MALE && (poke1.getGender() == PokemonSpecies.GENDER_FEMALE || poke1.getSpeciesName() == "Ditto"))
					{
						malePoke = poke2;
						femalePoke = poke1;
						return true;
					}
					else if(poke1.getGender() == PokemonSpecies.GENDER_NONE && poke2.getSpeciesName() == "Ditto")
					{
						malePoke = poke1;
						femalePoke = poke2;
						return true;
	...
	private int generateEggSpecies()
	{
		...
		if(femalePoke.getSpeciesName() != "Ditto")
			// Nidoran species
			if(femalePoke.getSpeciesName() == "NidoranF" || femalePoke.getSpeciesName() == "Nidorina" || femalePoke.getSpeciesName() == "Nidoqueen")
			{
	...
	private Pokemon generateHatchling(int species)
	{
		Pokemon hatchling;
			...
			if(femalePoke.getItemName() == "Everstone")
			{
				...
				nature = PokemonNature.getNature(random.nextInt(PokemonNature.getNatureNames().length)).getName();
			...
			for(int i = 0; i < 3; i++)
			{
				int randomNum = DataService.getBattleMechanics().getRandom().nextInt(2);
				attempt[i] = randomNum;
				if(i == 2)

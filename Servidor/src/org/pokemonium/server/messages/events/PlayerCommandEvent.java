...
import org.pokemonium.server.constants.UserClasses;
import org.pokemonium.server.constants.Weather;
...
public class PlayerCommandEvent implements MessageEvent
{
	...
	public void Parse(Session session, ClientMessage request, ServerMessage message)
	{
		...
		}else if (input.length() >= 10 && input.substring(0, 10).equalsIgnoreCase("createclan"))
		{
			message = new ServerMessage(ClientPacket.SHOWCLAN_WINDOW);
			session.Send(message);
			//GameServer._clanManager.createClan("DEVELOPERS", "DEVS", 220, 20, 70, session.getPlayer());
		}
		else
		{

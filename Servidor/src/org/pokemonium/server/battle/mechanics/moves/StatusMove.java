...
public class StatusMove extends PokemonMove
{
	...
	public Object clone()
	{
		...
		ret.m_statuses = ret.m_statuses.clone();
		ret.m_chances = ret.m_chances.clone();
		ret.m_attacker = ret.m_attacker.clone();
	...
	{
		return m_statuses.length != m_chances.length || m_chances.length != m_attacker.length;
	}
	...
	{
		if(idx < 0 || idx >= m_attacker.length)
			return;
	 ...
	 * then it can hit even types normally immune to the move's type. <br />
	 * <br />

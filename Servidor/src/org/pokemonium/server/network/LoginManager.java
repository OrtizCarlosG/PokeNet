...
import org.pokemonium.server.backend.entity.Bag;
import org.pokemonium.server.backend.entity.Player;
import org.pokemonium.server.backend.entity.Pokedex;
import org.pokemonium.server.backend.entity.PokemonBox;
...
import org.pokemonium.server.battle.mechanics.PokemonNature;
...
public class LoginManager implements Runnable
{
	...
	private Player getPlayerObject(ResultSet result)
	{
		try
		{
			...
			player.setUserID(result.getInt("id"));
			...
			if (GameServer._clanManager.isUserInClan(player))
			{
				ResultSet clanmemberData = m_database.query("SELECT * FROM pn_clanmembers WHERE username = '" + result.getString("username") + "'");
				clanmemberData.first();
				ResultSet clanData = m_database.query("SELECT * FROM pn_clans WHERE id = '" + clanmemberData.getInt("id") + "'");
				clanData.first();
			
				player.setUserClan(clanData.getString("clan_Name"));
				player.setUserClanTag(clanData.getString("clan_Tag"));
				player.setUserRank(clanmemberData.getInt("rank"));
			}
			return player;
		}
		catch(Exception e)
	...
	private void login(String username, char language, Session session, ResultSet result)
	{
		...
		if (doesAccountMemberExists(username))
		{
			System.out.println("INFO: " + player.getName() + " member status changed to 1.");
			m_database.query("UPDATE `pn_memberstatus` SET status = b'1' WHERE username = '" + MySqlManager.parseSQL(username) + "';");
		}
		else
		{
			System.out.println("INFO: " + player.getName() + " added to member status and changed to 1.");
			m_database.query("INSERT INTO `pn_memberstatus` (`id`, `username`, `status`) VALUES ('"+ player.getId() +"', '" + MySqlManager.parseSQL(username) + "', b'1');");
		}
		...
		/*Send boadcast stats*/
		ServerMessage message = new ServerMessage(ClientPacket.SERVER_BOADCASTSTATS);
		message.addBool(GameServer.getInstance().m_checkServerBroadcastMessage.isSelected());
		session.Send(message);
	}
	
	private Boolean doesAccountMemberExists(String username)
	{
		Boolean _result = false;
		ResultSet rs = m_database.query("SELECT * FROM `pn_memberstatus` WHERE username = '" + username + "';");
	    try {
			if(rs.first())
			{
				_result = true;

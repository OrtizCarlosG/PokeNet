...
public class PartyInfoDialog extends Widget
{
	...
	private Button[] m_ThrowPokeball;
	...
	public void initGUI()
	{
		for(int i = 0; i < 6; i++)
		{
			...
			try
			{
				...
				// Throw pokemon
				m_ThrowPokeball[i] = new Button();
				m_ThrowPokeball[i].setTheme("arrow_down"); 
				m_ThrowPokeball[i].setCanAcceptKeyboardFocus(false);
				m_ThrowPokeball[i].addCallback(new Runnable()
				{
					@Override
					public void run()
					{
						// Check if need walk
						if ( GameClient.getInstance().getOurPlayer().getLastPlayerX() == 0 || GameClient.getInstance().getOurPlayer().getLastPlayerY() == 0 )
						{
							// Aplly Dialog
							GameClient.getInstance().getGUI().invokeLater(new Runnable()
							{
								...
								public void run()
								{
									GameClient.getInstance().showMessageDialog("Walk a little before throw your pokemon!");
								}
							});
						}
						else
						{
							// Load image
							GameClient.getInstance().getOurPlayer().loadPokemonSpriteImage();
							
							// Apply Direction
							GameClient.getInstance().getOurPlayer().setPokemonDirection(GameClient.getInstance().getOurPlayer().getDirection());
							
							// Aplicar posicao
							GameClient.getInstance().getOurPlayer().setPokemonX(GameClient.getInstance().getOurPlayer().getLastPlayerX());
							GameClient.getInstance().getOurPlayer().setPokemonY(GameClient.getInstance().getOurPlayer().getLastPlayerY());
							
							// Apply sprite
							GameClient.getInstance().getOurPlayer().setPokemonsSprite(m_pokes[j].getSpriteNumber());
							
							/* Send message */
							ClientMessage message = new ClientMessage(ServerPacket.THROW_POKEMON);
							message.addInt(m_pokes[j].getSpriteNumber());
							GameClient.getInstance().getSession().send(message);
							...
							GameClient.getInstance().getGUI().invokeLater(new Runnable()
							{
								...
								public void run()
								{
									GameClient.getInstance().showMessageDialog("Go " + m_pokes[j].getName() + "!");
								}
							});
						}
				...
				m_container[i].add(m_ThrowPokeball[i]);
			}
		...
		for(int i = 0; i < 6; i++)
		{
			try
			{
				if(pokes[i] != null)
				{
					...
					m_ThrowPokeball[i].setVisible(true);
				}
				else
				{
					...
					m_ThrowPokeball[i].setVisible(false);
	...
	public void layout()
	{
		...
		for(int i = 0; i < 6; i++)
		{
			...
			m_ThrowPokeball[i].setSize(16,  16);
			m_ThrowPokeball[i].setPosition(150 + getInnerX(), 28 + getInnerY() + containerOffset);

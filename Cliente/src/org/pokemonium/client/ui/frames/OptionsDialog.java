...
import org.pokemonium.client.ui.components.Checkbox;
...
public class OptionsDialog extends ResizableFrame
{
	...
	public void initGUI()
	{
		...
		m_sync = new Button("Sync");
		m_sync.setCanAcceptKeyboardFocus(true);
		add(m_sync);
		
		m_sync.addCallback(new Runnable()
		{
			@Override
			public void run()
			{
				ClientMessage PlayerSync = new ClientMessage(ServerPacket.PLAYER_RESYNC);
				GameClient.getInstance().getSession().send(PlayerSync);
			}
		});
	...
	public void layout()
	{
		...
		m_sync.setSize(50, 25);
		m_sync.setPosition(10, 128);
		setSize(400, 160);
	}
}

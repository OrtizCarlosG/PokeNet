...
import org.pokemonium.client.backend.Animator;
import org.pokemonium.client.backend.BattleImageDatabase;
import org.pokemonium.client.backend.BattleManager;
import org.pokemonium.client.backend.ClientMap;
import org.pokemonium.client.backend.ClientMapMatrix;
...
import org.pokemonium.client.backend.time.TimeService;
import org.pokemonium.client.backend.time.WeatherService;
import org.pokemonium.client.backend.time.WeatherService.Weather;
import org.pokemonium.client.constants.Language;
import org.pokemonium.client.constants.Music;
import org.pokemonium.client.constants.ServerPacket;
import org.pokemonium.client.network.Connection;
import org.pokemonium.client.protocol.ClientMessage;
import org.pokemonium.client.ui.GUIPane;
import org.pokemonium.client.ui.HUD;
import org.pokemonium.client.ui.LoginScreen;
...
public class GameClient extends BasicGame
{
	...
	private final static String GAME_TITLE = "Pokemon Online";
	...
	private String m_language = Language.SPANISH;
	...
	private Image[] m_spritePokemonImageArray = new Image[500];
	...
	private void handleKeyPress(int key)
	{
		...
		{
			System.out.println("INFO: Tryed to start Interaction");
		...
		if(key ==KeyManager.getKey(Action.INTERACTION))
			System.out.println("Pressed interaction key");
	}
	...
	public void init(GameContainer gc) throws SlickException
	{
		...
		setPokemonSpriteFactory();
		ItemSpriteDatabase.Init();
		BattleImageDatabase.Init();
	...
	private void loadSprites()
	{
		try
		{
			...
			for(int ii = 1; ii < 494; ii++)
			{
				final String location = respath + "res/pokemon/overworld/normal/" + ii + ".png";
				...
				m_spritePokemonImageArray[ii] = new Image(location);
			}
		}
		catch(SlickException se)
		{
	...
	public void mousePressed(int button, int x, int y)
	{
		...
		if(button == 0)
		{
			...
			try
			{
				...
				if(getHUD().getNPCSpeech() == null && !root.getLoginScreen().isVisible() && !getHUD().getChat().hasKeyboardFocus() && !getHUD().hasMoveLearning())
				{
					ClientMessage message = new ClientMessage(ServerPacket.TALKING_START);
					m_session.send(message);
				}
				else{
					System.out.println("Mouse button is not avaliable");
				}
	...
	public void setPokemonSpriteFactory()
	{
		Player.setPokemonSpriteFactory(new PokemonSpriteFactory(m_spritePokemonImageArray));
	}
	...
	public void update(GameContainer gc, int delta) throws SlickException
	{
		...
		if(m_started)
		{
			...
			if(m_isNewMap && m_ourPlayer != null)
			{
				getGUIPane().hideHUD();
				getGUIPane().showLoadingScreen();
				m_mapMatrix.loadMaps(m_mapX, m_mapY, gc.getGraphics(), m_ourPlayer.getPokemonSprite());
	 ...
	 * Resync player
	 */
	public void resyncPlayer()
	{
		ClientMessage PlayerSync = new ClientMessage(ServerPacket.PLAYER_RESYNC);
		m_session.send(PlayerSync);
	}
}

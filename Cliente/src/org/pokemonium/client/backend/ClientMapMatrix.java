...
public class ClientMapMatrix
{
	...
	public String getMapName(int x, int y)
	{
		try
		{
			return m_mapMatrix[x][y].getName();
		}
		catch (Exception ex) 
		{
	 ...
	 * @param y
	 */
	public void loadMaps(int mapX, int mapY, Graphics g, int iPokemonID)
	{
		// Block player moves
		GameClient.getInstance().getOurPlayer().m_canMove = false;
		
		// Apply map
		GameClient.getInstance().getOurPlayer().m_Mapx = mapX;
		GameClient.getInstance().getOurPlayer().m_Mapy = mapY;
		...
		try
		{
			...
			{ // In case of emergencies, load english!
				try
				{
					BufferedReader reader = FileLoader.loadTextFile(respath + "res/language/english/NPC/" + mapX + "." + mapY + ".txt");
		...
		// Call resync player position
		GameClient.getInstance().resyncPlayer();
		
		// Set pokemon location
		GameClient.getInstance().getOurPlayer().setPokemonX(GameClient.getInstance().getOurPlayer().getX());
		GameClient.getInstance().getOurPlayer().setPokemonY(GameClient.getInstance().getOurPlayer().getY());
		
		// Apply id
		GameClient.getInstance().getOurPlayer().setPokemonsSprite(iPokemonID);
		
		// Unblock player moves
		GameClient.getInstance().getOurPlayer().m_canMove = true;
	}

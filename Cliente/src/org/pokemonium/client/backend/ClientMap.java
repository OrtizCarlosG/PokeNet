...
import org.pokemonium.client.constants.UserClass;
		...
		synchronized(m_mapMatrix.getPlayers())
		{
			try
			{
				...
				while(it.hasNext())
				{
					...
					{	
						// Check pokemon sprite
						if ( player.getPokemonSprite() != 0 && player.getCurrentPokemonImage() != null )
						{
							// Aplicar posição
							player.getCurrentPokemonImage().draw(m_xOffset + player.getPokemonX() - 13, m_yOffset + player.getPokemonY() - 23, 58, 68);
						}
						else if ( player.getPokemonSprite() != 0 )
						{
							// Carregar imagem
							player.loadPokemonSpriteImage();
						}
						
						// Draw the pokemon reflex
						if(m_curMap.shouldReflect(player))
						{
							...
							Image m_reflection = player.getCurrentPokemonImage().getFlippedCopy(false, true);
							m_reflection.setAlpha((float) 0.05);
							if(player.getPokemonSprite() != -0)
								m_reflection.draw(m_xOffset + player.getPokemonX() - 4, m_yOffset + player.getPokemonY() + 32);
							else
								m_reflection.draw(m_xOffset + player.getPokemonX() - 4, m_yOffset + player.getPokemonY() + 8);
						}
						
						// Draw the player
						if ( player.getSprite() != 0 )
						{
							player.getCurrentImage().draw(m_xOffset + player.getX() - 4, m_yOffset + player.getY());
						}
						
						// Draw the player reflex
						if(m_curMap.shouldReflect(player))
						{
							// If there's a reflection, flip the player's image, set
							// his alpha so its more translucent, and then draw it.
